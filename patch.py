    ...
    def parse(self, stream):
        ...
        for lineno, line in fe:
            ...
            if hunkbody:
                ...
                if (
                    hunkactual["linessrc"] > hunkinfo.linessrc
                    or hunkactual["linestgt"] > hunkinfo.linestgt
                ):
                    warning(
                        "extra hunk no.%d lines at %d for target %s"
                        % (nexthunkno, lineno + 1, self.target[nextfileno - 1])
                    )
                    # add hunk status node
                    self.hunks[nextfileno - 1].append(hunkinfo.copy())
                    ...
                    hunkinfo.linessrc == hunkactual["linessrc"]
                    ...
                    if (
                        ...
                        warning(
                            "inconsistent line ends in patch hunks for %s"
                            % self.source[nextfileno - 1]
                        )
                    if debugmode:
                        debuglines = dict(ends)
                        debuglines.update(
                            file=self.target[nextfileno - 1], hunk=nexthunkno
                        )
                        debug(
            ...
            if filenames:
                if line.startswith("--- "):
                    if nextfileno in self.source:
                        warning(
                            "skipping invalid patch for %s" % self.source[nextfileno]
                        )
                    ...
                    if nextfileno in self.source:
                        warning(
                            "skipping invalid patch with no target for %s"
                    ...
                    if nextfileno in self.target:
                        warning(
                            "skipping invalid patch - double target at line %d" % lineno
                        )
                        del self.source[nextfileno]
            ...
            for lineno, line in enumerate(f2fp):
                ...
                if lineno + 1 < hunk.startsrc + len(hunkfind) - 1:
                    if line.rstrip("\r\n") == hunkfind[hunklineno]:
                        ...
                        debug(
                            "hunk no.%d doesn't match source file %s"
                            % (hunkno + 1, filename)
                        )
                ...
                if lineno + 1 == hunk.startsrc + len(hunkfind) - 1:
                    debug(
                        "file %s hunk no.%d -- is ready to be patched"
                        % (filename, hunkno + 1)
                    )
                    hunkno += 1
                    validhunks += 1
...
if __name__ == "__main__":
    ...
    opt = OptionParser(
        usage="%prog [options] unipatch-file", version="python-patch %s" % __version__
    )
    opt.add_option(
        "-d",
        "--debug",
